...
class SummaryCallback(Callback): #用于在训练过程中对数据进行汇总，以便在MindInsight中进行可视化展示。它记录了训练过程中的平均奖励和损失，并定期将这些数据写入MindInsight的摘要文件中
...
class EvalRecordCallback(EvaluateCallback): #用于在训练过程中定期进行评估，并将评估结果记录下来。它会在每个评估周期结束时打印评估的总奖励，并将评估奖励记录下来以供后续分析和可视化
    r'''
